...
export default function Mobile() {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        const res = await fetch("/api/projects/get-all?type=mobile");
  ...
  return (
    <div className="w-full min-h-[70vh] bg-gradient-to-r from-yellow-200 via-white to-red-200 p-12 ">
      {loading ? (
          ...
          <div className="flex gap-2 lg:h-[50vh] flex-col lg:flex-row">
            <div className="flex-1 flex flex-col gap-4 justify-center py-4 ">
              <h1 className="xl:text-5xl text-3xl font-semibold text-red-700 drop-shadow-md lg:pb-8 pb-2">
                {" "}
                Discover The <span className="font-bold"> Mobile Apps</span> That We Are Proud Of
              </h1>
            ...
            <div className="flex flex-wrap gap-6 justify-center">
              {projects.map((project, index) => {
                return (
                  ...
                  >
                    <h3 className="absolute top-4 left-5 drop-shadow-lg text-xl font-semibold uppercase text-white">{project.name}</h3>
                    <img
